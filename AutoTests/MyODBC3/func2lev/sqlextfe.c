...
** Option       : --
**
** Purpose      : Auto Test DLL, Test von SQLEXTENDEDFETCH
**
******************************************************************************
*/
#ifndef WIN32
...
//*-------------------------------------------------------------------------
#define TESTNAME "SQLExtendedFetch"
#define TABLE "EXTFETCH"
...
#define TXTCOPY "SQLExtendedFetch String "
...
//*-------------------------------------------------------------------------
//| DoSQLExtendedFetch:
//|     This test case will test the SQLExtendedFetch access.
//|
//|
//| Returns:
...
/* ------------------------------------------------------------------------- */
	...
	UCHAR                   select[MAX_NAME_LEN+1+MAX_NAME_LEN];
...
/* ---------------------------------------------------------------------har- */
	...
	/* ***** SQL_FETCH_NEXT *** ----------------------------------------- */
	   while (1) {
		...
		DoPrintArray("SQL_FETCH_NEXT", crow, rgfRowStatus, sNumber_rc,
		             szString_rc, lpSrvr);
	    } else { break; }
	   }
	/* ***** SQL_FETCH_FIRST *** ---------------------------------------- */
	   rc = SQLExtendedFetch(*phstmt, SQL_FETCH_FIRST, 1, &crow, rgfRowStatus);
  	   ERRORCHECK(*phenv,*phdbc,*phstmt,lpSrvr,SQL_SUCCESS,rc,"SQLExtendedFetch");

	   if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
	     DoPrintArray("SQL_FETCH_FIRST", crow, rgfRowStatus, sNumber_rc,
		             szString_rc, lpSrvr);
	   }
	/* ***** SQL_FETCH_PRIOR *** ---------------------------------------- */
	   rc = SQLExtendedFetch(*phstmt, SQL_FETCH_NEXT, 1, &crow, rgfRowStatus);
	   ...
	   rc = SQLExtendedFetch(*phstmt, SQL_FETCH_NEXT, 1, &crow, rgfRowStatus);
  	   ERRORCHECK(*phenv,*phdbc,*phstmt,lpSrvr,SQL_SUCCESS,rc,"SQLExtendedFetch");
	   rc = SQLExtendedFetch(*phstmt, SQL_FETCH_PRIOR, 1, &crow, rgfRowStatus);
  	   ERRORCHECK(*phenv,*phdbc,*phstmt,lpSrvr,SQL_SUCCESS,rc,"SQLExtendedFetch");

	   if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
	     DoPrintArray("SQL_FETCH_PRIOR", crow, rgfRowStatus, sNumber_rc,
		             szString_rc, lpSrvr);
	   }
	/* ***** SQL_FETCH_LAST  *** ---------------------------------------- */
	   rc = SQLExtendedFetch(*phstmt, SQL_FETCH_LAST, 1, &crow, rgfRowStatus);
  	   ERRORCHECK(*phenv,*phdbc,*phstmt,lpSrvr,SQL_SUCCESS,rc,"SQLExtendedFetch");

	   if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
	     DoPrintArray("SQL_FETCH_LAST", crow, rgfRowStatus, sNumber_rc,
		             szString_rc, lpSrvr);
	   }
	/* ***** SQL_FETCH_ABSOLUTE *** ------------------------------------- */
	   rc = SQLExtendedFetch(*phstmt, SQL_FETCH_ABSOLUTE, ROW_POS, &crow, rgfRowStatus);
  	   ERRORCHECK(*phenv,*phdbc,*phstmt,lpSrvr,SQL_SUCCESS,rc,"SQLExtendedFetch");

	   if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
	     DoPrintArray("SQL_FETCH_ABSOLUTE", crow, rgfRowStatus, sNumber_rc,
		             szString_rc, lpSrvr);
	   }
	/* ***** SQL_FETCH_RELATIVE *** ------------------------------------- */
	   rc = SQLExtendedFetch(*phstmt, SQL_FETCH_RELATIVE, 0, &crow, rgfRowStatus);
  	   ERRORCHECK(*phenv,*phdbc,*phstmt,lpSrvr,SQL_SUCCESS,rc,"SQLExtendedFetch");

	   if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
	     DoPrintArray("SQL_FETCH_RELATIVE", crow, rgfRowStatus, sNumber_rc,
		             szString_rc, lpSrvr);
	   }
	/* ***** SQL_FETCH_BOOKMARK *** ------------------------------------- */
/* --- mysql ???
	   rc = SQLExtendedFetch(*phstmt, SQL_FETCH_BOOKMARK, ROW_POS+5, &crow, rgfRowStatus);
  	   ERRORCHECK(*phenv,*phdbc,*phstmt,lpSrvr,SQL_SUCCESS,rc,"SQLExtendedFetch");

	   if (rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
	     DoPrintArray("SQL_FETCH_BOOKMARK", crow, rgfRowStatus, sNumber_rc,
		             szString_rc, lpSrvr);
	   }
*/
	/* ------------------------------------------------------------------ */
