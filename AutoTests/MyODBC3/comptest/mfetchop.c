...
/* ---------------------------------------------------------------------har- */
	...
	lstrcat(insert," VALUES ('<< Dies ist ein MFETCH-Test >>', 999, '1994-11-07')");
	...
	while (1)  {
		...
		rc = SQLGetData(*phstmt,3,SQL_C_DATE,&typ_date,0,&cbDate);
		ERRORCHECK(*phenv,*phdbc,*phstmt,lpSrvr,SQL_SUCCESS,rc,"SQLGetData?");
