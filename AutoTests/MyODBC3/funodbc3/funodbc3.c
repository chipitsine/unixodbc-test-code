...
** Program      : FUNODBC3.C
**
** Purpose      : FUNCTIONODBC 3.0 Auto Test DLL
**
****************************************************************************
*/
//*-------------------------------------------------------------------------
...
//|                                     FUNCTION.C      This shource file with test code
//|                                     FUNCTION.H      Include files with defines, macros, and prototypes
//|                                     FUNCTION.RC     Resource file for string tables
...
SWORD FAR PASCAL DoSQLColAttribute(HENV FAR * phenv, HDBC FAR * phdbc,
				  HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoSQLCopyDesc(HENV FAR * phenv, HDBC FAR * phdbc,
				  HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
...
SWORD FAR PASCAL DoSQLFreeHandle(HENV FAR * phenv, HDBC FAR * phdbc,
				  HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoSQLGetConnectAttr(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
...
SWORD FAR PASCAL DoSQLGetDiagRec(HENV FAR * phenv, HDBC FAR * phdbc,
				  HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoSQLGetDescField(HENV FAR * phenv, HDBC FAR * phdbc,
				  HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
...
SWORD FAR PASCAL DoSQLGetInfo(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoSQLGetStmtAttr(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoSQLSetConnectAttr(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoSQLSetDescField(HENV FAR * phenv, HDBC FAR * phdbc,
				  HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoSQLSetDescRec(HENV FAR * phenv, HDBC FAR * phdbc,
				  HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoSQLSetEnvAttr(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
...
SWORD FAR PASCAL DoSQLTablePrivileges(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoSQLNumericStructI(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
...
// -------------------- ----------------------- ------------------------
	idsSQLAllocHandle,      idsSQLAllocHandleDesc,         DoSQLAllocHandle,
	idsSQLCloseCursor,      idsSQLCloseCursorDesc,         DoSQLCloseCursor,
	idsSQLColAttribute,     idsSQLColAttributeDesc,        DoSQLColAttribute,
	idsSQLCopyDesc,         idsSQLCopyDescDesc,            DoSQLCopyDesc,
	idsSQLDescribeParam,    idsSQLDescribeParamDesc,       DoSQLDescribeParam,
	idsSQLEndTran,          idsSQLEndTranDesc,             DoSQLEndTran,
	idsSQLFetchScroll,      idsSQLFetchScrollDesc,         DoSQLFetchScroll,
	idsSQLFreeHandle,       idsSQLFreeHandleDesc,          DoSQLFreeHandle,
	idsSQLGetConnectAttr,   idsSQLGetConnectAttrDesc,      DoSQLGetConnectAttr,
	idsSQLGetDiagField,     idsSQLGetDiagFieldDesc,        DoSQLGetDiagField,
	idsSQLGetDiagRec,       idsSQLGetDiagRecDesc,          DoSQLGetDiagRec,
	idsSQLGetDescField,     idsSQLGetDescFieldDesc,        DoSQLGetDescField,
	idsSQLGetDescRec,       idsSQLGetDescRecDesc,          DoSQLGetDescRec,
	idsSQLGetEnvAttr,       idsSQLGetEnvAttrDesc,          DoSQLGetEnvAttr,
	idsSQLGetFunctions,     idsSQLGetFunctionsDesc,        DoSQLGetFunctions,
	idsSQLGetInfo,          idsSQLGetInfoDesc,             DoSQLGetInfo,
	idsSQLGetStmtAttr,      idsSQLGetStmtAttrDesc,         DoSQLGetStmtAttr,
	idsSQLSetConnectAttr,   idsSQLSetConnectAttrDesc,      DoSQLSetConnectAttr,
	idsSQLSetDescField,     idsSQLSetDescFieldDesc,        DoSQLSetDescField,
	idsSQLSetDescRec,       idsSQLSetDescRecDesc,          DoSQLSetDescRec,
	idsSQLSetEnvAttr,       idsSQLSetEnvAttrDesc,          DoSQLSetEnvAttr,
	idsSQLSetStmtAttr,      idsSQLSetStmtAttrDesc,         DoSQLSetStmtAttr,
	idsSQLColumnPrivileges, idsSQLColumnPrivilegesDesc,    DoSQLColumnPrivileges,
	idsSQLColumns,          idsSQLColumnsDesc,             DoSQLColumns,
	idsSQLForeignKeys,      idsSQLForeignKeysDesc,         DoSQLForeignKeys,
	idsSQLPrimaryKeys,      idsSQLPrimaryKeysDesc,         DoSQLPrimaryKeys,
	idsSQLProcedures,       idsSQLProceduresDesc,          DoSQLProcedures,
	idsSQLProcedureColumns, idsSQLProcedureColumnsDesc,    DoSQLProcedureColumns,
	idsSQLSpecialColumns,   idsSQLSpecialColumnsDesc,      DoSQLSpecialColumns,
	idsSQLStatistics,       idsSQLStatisticsDesc,          DoSQLStatistics,
	idsSQLTables,           idsSQLTablesDesc,              DoSQLTables,
	idsSQLTablePrivileges,  idsSQLTablePrivilegesDesc,     DoSQLTablePrivileges,
	idsSQLNumericStructI,	idsSQLNumericStructIDesc,      DoSQLNumericStructI,
	idsSQLNumericStructS,	idsSQLNumericStructSDesc,      DoSQLNumericStructS,
	idsSQLNumericStructX,	idsSQLNumericStructXDesc,      DoSQLNumericStructX
	};
...
void EXTFUN AutoTestFunc(lpSERVERINFO lpSrvr)
{
	...
	char            szName[10*AUTO_MAX_TESTCASE_NAME+6];       // Room for NULL and \r\n
...
/* ---------------------------------------------------------------------har- */
 if (actrc != SQL_SUCCESS) {
	api_rc = SQLGetDiagRec(HandleType, Handle, RecNumber,
	                       sqlstate, &esq_sql_code, error_txt,
	                       len_error_txt, &used_error_txt);
	if ( opt != NO_OUTPUT) {
	 if (api_rc == SQL_NO_DATA_FOUND) {
	   szLogPrintf(lpSrvr, FALSE,"\tSQLGetDiagRec -> (SQL_NO_DATA_FOUND)\r\n");
	 } else {
	   if (actrc < 0) lstrcpy(outtxt,"Error");
	     else lstrcpy(outtxt,"Warning");
	   sprintf(buffer,"sqlstate: %s",sqlstate);
	   szLogPrintf(lpSrvr, FALSE,"\tSQLGetDiagRec (%s) -> %s\r\n", outtxt, buffer);
	   sprintf(buffer,"esq_sql_code: %ld",esq_sql_code);
	   szLogPrintf(lpSrvr, FALSE,"\tSQLGetDiagRec (%s) -> %s\r\n", outtxt, buffer);
	   sprintf(buffer,"error_txt: %s", error_txt);
	   szLogPrintf(lpSrvr, FALSE,"\tSQLGetDiagRec (%s) -> %s\r\n", outtxt, buffer);
