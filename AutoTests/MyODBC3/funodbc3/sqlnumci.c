...
** Option       : --
**
** Purpose      : Auto Test DLL, Test von SQL_NUMERIC_STRUCT (put-/getdata)
**
****************************************************************************
*/
#ifndef WIN32
...
//*-------------------------------------------------------------------------
#define TESTNAME "SQLNUMERICSTRUCTI"
...
/* ------------------------------------------------------------------------- */
    ...
    SDWORD  local_value;
    SDWORD  local_long;
    SDOUBLE local_double=0;
    SDOUBLE save_double=0;
    ...
    SWORD   exponent = 0;
    char    local_string[API_CHAR_LENGTH_CHAR+1];
/* ------------------------------------------------------------------------- */
    char *sqltable  = "NUMCSTRUCTI";
...
/* ------------------------------------------------------------------------- */
    ...
    for (i=0; i < MAX_INSERT; i ++) {
      ...
      iValueInput = i+1;
      local_double=save_double;
      sprintf(local_string, "%f",local_double);
      ...
      local_long=atol(local_string);
      sprintf(buffer, "%d",local_long);
      for (ixx=0; ixx < sizeof(local_long); ixx++) {
           local_numeric_input.val[ixx]=(char)((local_long >> (ixx*8)) & 0xff);
      }

      sprintf(buffer," keyx:%d - numv: %.3f (prec:%d scale:%d sign:%d)",
		iValueInput, local_double, local_numeric_input.precision,
