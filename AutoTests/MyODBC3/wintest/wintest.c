...
//|                                     DOSTEST.C       This shource file with test code
//|                                     DOSTEST.H       Include files with defines, macros, and prototypes
//|                                     DOSTEST.RC      Resource file for string tables
...
#define COMPARE_FILE
#define PRINT_WINDOW
#define TRACEFILE "WST"
#define TRACEEXTN ".OUT"
#define TRACETMP  ".TMP"
...
#ifndef WIN32
void FatalAppExit( int status, char * msg ) 
{
	fprintf( stderr, msg );
	exit( status );
}
...
void PutComp(UCHAR buffer[1024], UCHAR prg_name[13], lpSERVERINFO lpSrvr);
...
SWORD FAR PASCAL DoGetDataR(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoGetDataS(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoLongDatatype0(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoLongDatatype1(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoLongDatatype2(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoLongDatatype3(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoLongDatatype4(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoLongDatatype5(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoLongDatatype6(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoLongDatatype7(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
...
SWORD FAR PASCAL DoResultMax(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
SWORD FAR PASCAL DoSingleIndex(HENV FAR * phenv, HDBC FAR * phdbc,
				HSTMT FAR * phstmt, lpSERVERINFO lpSrvr);
...
// -------------------- ----------------------- ------------------------
	idsCursorName,          idsCursorNameDesc,              DoCursorName,
	idsDatatype,            idsDatatypeDesc,                DoDatatype,
	idsDBProc,              idsDBProcDesc,                  DoDBProc,
	idsFloatNumbers,        idsFloatNumbersDesc,            DoFloatNumbers,
	idsFixedNumbers,        idsFixedNumbersDesc,            DoFixedNumbers,
	idsGetDataR,	        idsGetDataRDesc, 	        DoGetDataR,
	idsGetDataS,	        idsGetDataSDesc, 	        DoGetDataS,
	idsLongDatatype0,       idsLongDatatype0Desc,           DoLongDatatype0,
	idsLongDatatype1,       idsLongDatatype1Desc,           DoLongDatatype1,
	idsLongDatatype2,       idsLongDatatype2Desc,           DoLongDatatype2,
	idsLongDatatype3,       idsLongDatatype3Desc,           DoLongDatatype3,
	idsLongDatatype4,       idsLongDatatype4Desc,           DoLongDatatype4,
	idsLongDatatype5,       idsLongDatatype5Desc,           DoLongDatatype5,
	idsLongDatatype6,       idsLongDatatype6Desc,           DoLongDatatype6,
	idsLongDatatype7,       idsLongDatatype7Desc,           DoLongDatatype7,
	idsLongDatatype8,       idsLongDatatype8Desc,           DoLongDatatype8,
	idsLongDatatype9,       idsLongDatatype9Desc,           DoLongDatatype9,
	idsLongDatatypeA,       idsLongDatatypeADesc,           DoLongDatatypeA,
	idsMFetchTest,	        idsMFetchTestDesc,	        DoMFetchTest,
	idsMultIndex,	        idsMultIndexDesc,	        DoMultIndex,
	idsReConnect,	        idsReConnectDesc,	        DoReConnect,
	idsResultMax,	        idsResultMaxDesc,	        DoResultMax,
	idsSingleIndex,	        idsSingleIndexDesc,	        DoSingleIndex,
	idsTimestamp,	        idsTimestampDesc,	        DoTimestamp,
	idsTimestampfr,	        idsTimestampfrDesc,	        DoTimestampfr,
	};
...
void PutComp(buffer, prg_name, lpSrvr)
UCHAR buffer[256];
UCHAR prg_name[13];
lpSERVERINFO lpSrvr;
{
...
lstrcpy(file,TRACEFILE);
lstrcat(file,prg_name);
lstrcat(file,TRACETMP);
...
#ifdef WIN32
if (!file_handle) {
     file_handle = OpenFile((UCHAR FAR *) file,
			    &OfStruct, OF_CREATE );
     }
     else {
     file_handle = OpenFile((UCHAR FAR *) file,
			    &OfStruct, OF_REOPEN | OF_WRITE);
     }
     if (file_handle == 0)
	FatalAppExit(0,"trace file won't open!!!!!");
     _llseek(file_handle, 0L, 2);
     _lwrite(file_handle,buffer,strlen(buffer));
     _lclose(file_handle);
#else
	 file_handle = open( file, O_CREAT | O_RDWR );
	 if ( file_handle == -1 ) {
	 	file_handle = open( file, O_RDWR );
	 }
	 if ( file_handle == -1 ) {
		FatalAppExit(0,"trace file won't open!!!!!");
	 }
     lseek(file_handle, 0L, 2);
