...
#define _tcsnccmp   strncasecmp
#define _tstrtime(x) timgtime(x)
#define min(x,y)    ((x>y)?(y):(x))
#define _sntprintf  snprintf
#define _tcsnccat    strncat
#define _tcschr     strchr
#define _ltot(v,s,r)  sprintf(s,"%d",v)
...
char *timgtime(char *s)
{
    time_t tim = time( NULL );
    return strftime( s, 20, "%H%M%S", localtime( &tim ));
}

//--------------------------------------------------------------------------------------
//        static
//--------------------------------------------------------------------------------------
static CHAR  vszFile[]                         = TEXT(__FILE__);
...
SQLCHAR szParamName[MAX_STRING_SIZE]=TEXT("Parameter Name");
...
CHAR szInsertStmt[MAX_STRING_SIZE];

/*-----------------------------------------------------------------------
//      Function:               Supported
//-----------------------------------------------------------------------*/

 UWORD   Supported(UWORD uwSQLFunc)
{
	UWORD           fLevel2=FALSE;
...
void  BinToHex(PTR rgbValue, SWORD cbValue, LPTSTR szHexString)
{
	UWORD ib;
	LPTSTR pch;
	CHAR rgchDigits[] = TEXT("0123456789ABCDEF");
...
/*-----------------------------------------------------------------------*/
 ...
 int  SearchForError(HENV henv, HDBC hdbc,HSTMT hstmt,LPTSTR szFindState)
{
...
/*-----------------------------------------------------------------------*/
 ...
 VOID  szWrite(LPTSTR szStr, BOOL fNewLine)
{
...
VOID  GetErrors(HENV henv,HDBC hdbc,HSTMT hstmt)
{
	...
	{
		sprintf(largebuf,TEXT("\t\t\tState: %s"), szState);
		szWrite(largebuf, TRUE);
		sprintf(largebuf,TEXT("\t\t\tError: %s"), buf);
		szWrite(largebuf, TRUE);
	}
...
VOID  GetDiagRecs(SWORD fHandleType,SQLHANDLE hHandle)
{
	CHAR           szMessageText[XLARGEBUFF]=TEXT(""),
				szBuff[XLARGEBUFF]=TEXT("");
	CHAR           szSQLState[100]=TEXT("");
	RETCODE rc=SQL_SUCCESS;
	SWORD           irecRecNumber=0;
	SDWORD  fNativeError=0;
	SWORD           cchBufferLength=sizeof(szMessageText)/sizeof(CHAR),
				cbTextLength=0;
	SDWORD  sdNumRecs=0;
	...
	for(irecRecNumber=1;(irecRecNumber <= sdNumRecs) && RC_SUCCESSFUL(rc);irecRecNumber++)
	{
		...
		if (RC_SUCCESSFUL(rc))
		{
			sprintf(szBuff,TEXT("\t\t\tState: %s"), szSQLState);
			szWrite(szBuff, TRUE);
			sprintf(szBuff,TEXT("\t\t\tError: %s"), szMessageText);
			szWrite(szBuff, TRUE);
		}
...
/*-----------------------------------------------------------------------*/
/*      Function:               RetcodeToCHAR                            */
/*-----------------------------------------------------------------------*/

LPTSTR   RetcodeToCHAR(RETCODE retcode, LPTSTR buf)
{
	switch (retcode) 
		{
		case SQL_SUCCESS:
...
int   ReturnCheck(RETCODE retExpected, RETCODE retReceived, LPTSTR szFunction, 
								LPTSTR szFile, int iLine,BOOL fDiag)
{
	...
	sprintf(buf,TEXT("\t\t\t%s: %d"), szFile, iLine);
	szWrite(buf, TRUE);
...
int   ErrsHandle(SWORD fHandleType,SQLHANDLE hHandle, RETCODE retExpected,
...
BOOL  FindError(SWORD fHandleType, LPTSTR szFindState)
{
...
{
	LPTSTR   pch=NULL;
	CHAR szExpBuf[MAX_STRING_SIZE]=TEXT("");
	CHAR * stopstr;
	...
	if(pch) 
	{
		...
		{
			BOOL bCompare;
			CHAR * pstr=szExpBuf;  // Pointer to start of expected string
...
VOID  GetSomeData(HSTMT lhstmt, LPTSTR szTableName,BOOL fBindCol,
								FIELDINFO  rgField)
{
	RETCODE rc=SQL_SUCCESS;
	SDWORD  cbValue=0;
	CHAR           szQuery[XLARGEBUFF]=TEXT(""),
				szBuff[XLARGEBUFF]=TEXT("");
	CHAR           szErrMsg[MAX_STRING_SIZE]=TEXT(""),
				szDataItem[XLARGEBUFF]=TEXT("");
	LPTSTR   pch=NULL;

	pch = qtMakeData(0, 0,&rgField, szDataItem);

	sprintf(szQuery,szSELECTSTAR,szTableName);
	rc=SQLExecDirect(lhstmt,szQuery,SQL_NTS);
//      RETCHECK(SQL_SUCCESS,rc,szSQLEXECDIRECT);

	if (fBindCol)
	{
		rc = SQLBindCol(lhstmt, 1, SQL_C_CHAR, szBuff, sizeof(szBuff),&cbValue);
...
//      RETCHECK(SQL_SUCCESS, rc,szSQLFETCH);
	...
	if (!fBindCol)
	{
		rc = SQLGetData(lhstmt,1, SQL_C_CHAR,szBuff, sizeof(szBuff), NULL);
...
/*-----------------------------------------------------------------------*/
 ...
 RETCODE  SelectFromTable(QTSTRUCT *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
	CHAR           szQuery[LARGEBUFF];

	sprintf(szQuery,TEXT("select %s from %s"), 
	//sprintf(szQuery,TEXT("select %s from %s order by %s"), 
...
/*-----------------------------------------------------------------------*/
 ...
 RETCODE  SelectFromTableFetch(QTSTRUCT *lpqt, CHAR* szColName)
{
	RETCODE rc=SQL_SUCCESS;
	CHAR           szQuery[LARGEBUFF];
...
//-----------------------------------------------------------------------
 ...
 BOOL  SetStatementAttributes(UWORD fAttr, PTR rgbValue)
{
	RETCODE rc=SQL_SUCCESS;
	CHAR           szAPI[MEDBUFF];
	BOOL            fFailed=FALSE;
...
//-----------------------------------------------------------------------
 ...
 BOOL  SetConnectionAttributes(HDBC lhdbc,UWORD fAttr, UDWORD rgbValue)
{
	RETCODE rc=SQL_SUCCESS;
	CHAR           szAPI[MEDBUFF];
	BOOL            fFailed=FALSE;
...
BOOL  Is3XDriver(HDBC hdbc,QTSTRUCT *lpqt)
{
	RETCODE         rc=SQL_SUCCESS;
	LPTSTR          pch=NULL;
...
BOOL   Connect(lpSERVERINFO lpSI,QTSTRUCT  *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
	SDWORD sdwAppVer;
...
/* The signal for test to use it's
	...
	sprintf(lpqt->sz, TEXT("dsn=%s;uid=%s;pwd=%s;"),
		lpSI->szValidServer0,
		lpSI->szValidLogin0,
...
BOOL   IgnoreType(LPTSTR szType,SWORD wSQLType)
{
	BOOL fIgnore=FALSE;
...
*/
	...
	if (!_tcscmp(szType,TEXT("LONG RAW")) ||
		 !_tcscmp(szType,TEXT("LONG VARCHAR")) ||
		 !_tcscmp(szType,TEXT("LONG VARCHAR FOR BIT DATA")))
		{
		fIgnore=TRUE;
		}
...
VOID  SetErrorCondition(SWORD fHandleType)
{
	RETCODE rc=SQL_SUCCESS;
...
VOID  CheckData(SWORD fHandleType,UWORD uwDescField, PTR rgbValue,LPTSTR szDataSourceName)
{
	CHAR                   szBuff[MEDBUFF]=TEXT("");
		...
		case SQL_DIAG_RETURNCODE:
			{
				...
				if (rcRec!= SQL_ERROR)
				{
					sprintf(szBuff,TEXT("\t\t\tSQL_DIAG_RETURN_CODE\tReceived: %d\tExpected: %d"),rcRec,-1);
				}
			}
			break;
		...
		case SQL_DIAG_SQLSTATE:
			{
				...
				{
					sprintf(szBuff,TEXT("\t\t\tSQL_DIAG_SQLSTATE\tReceived: %s\tExpected: %s"),(LPTSTR)rgbValue,pszExpState);
				}
			}
			break;
		case SQL_DIAG_CLASS_ORIGIN:
			{
				if (_tcscmp((LPTSTR)rgbValue,szISO))
					sprintf(szBuff,TEXT("\t\t\tSQL_DIAG_CLASS_ORIGIN\tReceived: %s\tExpected: %s"),(LPTSTR)rgbValue,szISO);
			}
			break;
		...
		case SQL_DIAG_NUMBER:
			{
				...
				if (cRecs != 1)
					sprintf(szBuff,TEXT("\t\t\tSQL_DIAG_NUMBER\tReceived: %d\tExpected: %d"),cRecs,1);
			}
			break;
...
void  TestDiagRec()
{
	...
	CHAR                   szDataSourceName[MAX_STRING_SIZE] = TEXT("");
	CHAR                   szState[SQL_SQLSTATE_SIZE+1];
	CHAR                   szErrorMsg[SQL_MAX_MESSAGE_LENGTH];
	CHAR                   szAPI[MEDBUFF]=TEXT("");
	CHAR                   szBuff[MEDBUFF]=TEXT("");
	...
	for (iHandle=0; iHandle < cHandles; iHandle++)
	{
		...
		if(fDiagRecSupported)
		{
			...
			{
				sprintf(szBuff,TEXT("\t\t\tReceived: %s\tExpected: %s"),szState,       pszExpState);
...
void  TestDiagField()
{
	...
	CHAR                   szDataSourceName[MAX_STRING_SIZE] = TEXT("");
	CHAR                   rgbValue[SQL_MAX_MESSAGE_LENGTH];
	CHAR                   szAPI[MEDBUFF]=TEXT("");
	CHAR                   szBuff[MEDBUFF]=TEXT("");
	RETCODE         rcExp=SQL_SUCCESS;
...
//------------------------------------------------------------------------------
void CompareAnsiUnicodeResults(SQLHSTMT hstmtA, SQLHSTMT hstmtW, SQLCHAR * szAPI)
{
    return;
}
...
/*----------------------------------------------------------------------------*/
void  TestMixedAnsiUnicode(QTSTRUCT * lpqt)
{
    return;
...
void   SetTableName(QTSTRUCT  *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
	UWORD           cTableName;
	CHAR           szTime[MEDBUFF]=TEXT(""),
				szTmpBuff[MEDBUFF],
				*pTime=NULL;
...
UWORD   GetTypeInfo(FIELDINFO  *rgFields,QTSTRUCT  *lpqt)
{
	RETCODE         rc=SQL_SUCCESS;
	UWORD       cFieldName;
...
/* TO DO: add checks in here for dos file types GetInfo SQL_QUALIFIER_NAME_SEPARATER */
	...
	for(i = 0; i < MAX_TYPES_SUPPORTED; i++)
	{
		...
		*rgFields[i].szType = *rgFields[i].szLength = *rgFields[i].szParams = TEXT('\0');
		...
		sprintf(rgFields[i].szFieldName,TEXT("c%02u"), i+COLNAME_START);
		...
		/* Max length */
		rc = SQLGetData(hstmt, 3, SQL_C_CHAR,
		...
		/* prefix */
		rc = SQLGetData(hstmt, 4, SQL_C_CHAR,
		...
		/* create params */
		rc = SQLGetData(hstmt, 6, SQL_C_CHAR,
...
BOOL   BuildCreateStmt(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,UWORD cTypes)
{
	RETCODE         rc=SQL_SUCCESS;
	CHAR        szNum[MAX_NUM_BUFFER];   /* used with _ttoi and itoa conversions */
	SWORD       wNum;
	UWORD       i;
	SDWORD      sdw;
	LPTSTR       pch=NULL;
	...
	for(i = 0; i < cTypes; i++) {
		...
		if(rgFields[i].szParams == NULL || _tcslen(rgFields[i].szParams) == 0)
			*lpqt->szParam = TEXT('\0');
			...
			sprintf(lpqt->szParam,TEXT("(%s)"), rgFields[i].szLength);
		else
			{
			_tcscpy(szNum, rgFields[i].szLength);
			sprintf(lpqt->szParam,TEXT("(%s, %d)"), rgFields[i].szLength,
		...
		sprintf(&lpqt->sz[_tcslen(lpqt->sz)],TEXT(" %s %s %s ,"),
...
BOOL   BuildInsertStmt(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,UWORD cTypes,
	DSTRUCT  *lpd,SDWORD  *cColsSelected,BOOL fBindParameter)
{
	...
	for(i = 0; i < cTypes; i++) {
		...
		if(fColNames) 
			...
			sprintf(lpqt->sz,TEXT("insert into %s (%s) VALUES(%s)"),
...
void   DropTable(LPTSTR szTableName, BOOL fSilent)
{               
	CHAR           szBuff[100];
...
void   CleanUp(DSTRUCT  *lpd,QTSTRUCT  *lpqt,FIELDINFO  *rgFields,
...
BOOL   BindAllColumns(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,UWORD cTypes,
	DSTRUCT  *lpd)
{
//-----------------------------------------------------------------------
//      Function:               BindAllColumns
	...
	for(i = 0, w = 0; i < cTypes; i++) 
		...
		switch(rgFields[i].wSQLType) 
			...
			case SQL_NUMERIC:
			case SQL_DECIMAL:
				rc = SQLBindCol(hstmt, w, SQL_C_CHAR,
							lpd[i].data, MAX_STRING_SIZE*sizeof(SQLCHAR), &lpd[i].cb);
				break;
			default:
				rc = SQLBindCol(hstmt, w, SQL_C_DEFAULT,
...
BOOL   BindFetchColumns(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,UWORD cTypes,
	DSTRUCT  *lpdFetch)
{
	UWORD           i,
				w;
...
void   TestMetaData(lpSERVERINFO lpSI, QTSTRUCT  *lpqt,FIELDINFO  *rgFields,UWORD cTypes)
{
	SWORD    wNum;
	UWORD    w;
	UWORD    i;
	...
	for(i = 0, w = 0; i < cTypes; i++) 
		...
		rc = SQLDescribeCol(hstmt, w, lpqt->buf,
			sizeof(lpqt->buf)/sizeof(CHAR), NULL, &wType, &dwLen, &wNum, &wNull);
...
void  RetrieveData(QTSTRUCT *lpqt,FIELDINFO *rgFields,
	...
	for(ind = 0; ;ind++) 
		...
		for(i = 0; i < cTypes; i++) 
			...
			switch(rgFields[i].wSQLType) 
				...
				case SQL_DECIMAL:
				case SQL_NUMERIC:
					if(lpd[i].cb > MAX_STRING_SIZE*sizeof(CHAR))
						DISPLAYERROR(szRETDATA,szINCORRECTOUTLEN);

					// Chg _tcslen to strlen below pending spec decision
					if((lpd[i].cb / sizeof(CHAR)) > (UDWORD)_tcslen(lpd[i].data))
						DISPLAYERROR(szRETDATA,szINCORRECTOUTLEN);
				...
				case SQL_LONGVARCHAR:
					...
					if(lpd[i].cb / sizeof(SQLCHAR) > _tcslen(lpd[i].data))
						DISPLAYERROR(szRETDATA,szINCORRECTOUTLEN);
				...
				case SQL_CHAR:
					if(lpd[i].cb > MAX_STRING_SIZE*sizeof(CHAR) && rc == SQL_SUCCESS)
						DISPLAYERROR(szRETDATA,TEXT("incorrect return code for outlen"));

					if(((lpd[i].cb / sizeof(CHAR)) != (UDWORD)rgFields[i].precision) && rc == SQL_SUCCESS)
						DISPLAYERROR(szRETDATA,szINCORRECTOUTLEN);
...
void   TestData(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,
	UWORD cTypes,DSTRUCT  *lpd)
{
...
void   CreateParamQuery(QTSTRUCT *lpqt, FIELDINFO *rgFields,UWORD cTypes)
{       
	RETCODE rc=SQL_SUCCESS;
	UWORD           i;
	LPTSTR  pch=NULL;
	...
	sprintf(szQuery,TEXT("select %s from %s where "), lpqt->szColNames,lpqt->szTableName);
	...
	for(i = 0, w = 0,ind=0; i < cTypes; i++) 
			...
			pch && *pch) 
		{
			...
			switch(rgFields[i].wSQLType)
			{
				...
				case SQL_FLOAT:
				case SQL_DOUBLE:
					sprintf(&szQuery[_tcslen(szQuery)],TEXT("%s < ? + 1 and "),
						rgFields[i].szFieldName);
					break;
				default:
					sprintf(&szQuery[_tcslen(szQuery)],TEXT("%s = ? and "),
						rgFields[i].szFieldName);
			}
...
void   TestSearchedQuery(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,
	UWORD cTypes,DSTRUCT  *lpd,BOOL fBindParameter)
{
	...
	sprintf(lpqt->sz, TEXT("select %s from %s where "), lpqt->szColNames,
		lpqt->szTableName);
	...
	for(i = 0, w = 0,ind=0; i < cTypes; i++) 
		...
		if ((rgFields[i].fSearchable == SQL_SEARCHABLE ||
			...
			switch(rgFields[i].wSQLType) {
				...
				case SQL_FLOAT:
				case SQL_DOUBLE:
					sprintf(&lpqt->sz[_tcslen(lpqt->sz)], TEXT("%s < ? + 1 and "),
						rgFields[i].szFieldName);
					break;
				default:
					sprintf(&lpqt->sz[_tcslen(lpqt->sz)], TEXT("%s = ? and "),
					rgFields[i].szFieldName);
			}
	...
	for(i = 0; ; i++) 
		...
		if(*pch) 
			{
			udw = _tcslen(pch);
			rc = SQLPutData(hstmt, pch, (SDWORD)_tcslen(pch)*sizeof(SQLCHAR));
			RETCHECK(SQL_SUCCESS, rc, szSQLPUTDATA);
			} 
		else
			{
...
void   TestLargeQuery(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,
	UWORD cTypes,DSTRUCT  *lpd)
{
	RETCODE rc=SQL_SUCCESS;
	UWORD           ind=0,
	...
	/* > 1K query */
	sprintf(szQuery,TEXT("select %s from %s where "), lpqt->szColNames,
		lpqt->szTableName);
	...
	while(_tcslen(szQuery) < 1024L) 
		...
		switch(rgFields[i].wSQLType) 
			...
			case SQL_FLOAT:
			case SQL_DOUBLE:
				sprintf(&szQuery[_tcslen(szQuery)],TEXT("%s < %s + 1 and "),
					rgFields[i].szFieldName, pch);
				break;
			default:
				sprintf(&szQuery[_tcslen(szQuery)],TEXT("%s = %s%s%s and "),
...
void   TestSQLTables(QTSTRUCT  *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
	...
	for(i = 0;; i++) 
		...
		rc = SQLGetData(hstmt, 3, SQL_C_CHAR, lpqt->sz, sizeof(lpqt->sz), NULL);
...
void   TestSQLSpecialCols(QTSTRUCT  *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
	UWORD           i;
...
void   VerifyIndex(QTSTRUCT  *lpqt,UWORD fIndex)
{
	RETCODE rc=SQL_SUCCESS;
	BOOL            fFoundTable=FALSE;
	UWORD           i;
	...
	for(i = 0;; i++) 
		...
		rc = SQLGetData(hstmt, 3, SQL_C_CHAR, lpqt->sz, sizeof(lpqt->sz),
			&sdw);
		RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA);
		if (_tcsicmp(lpqt->sz, lpqt->szTableName) == 0)
			{
			rc = SQLGetData(hstmt, 6, SQL_C_CHAR, lpqt->sz, sizeof(lpqt->sz),
				&sdw);
			RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA);
...
BOOL   TestSQLStatistics(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,
	UWORD cTypes,DSTRUCT  *lpd,UWORD fIndex)
{
	...
	sprintf(lpqt->sz,TEXT("create unique index %s on %s (%s)"),
...
void   TestLikeQuery(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,
	UWORD cTypes,DSTRUCT  *lpd)
{
	RETCODE rc=SQL_SUCCESS;
	UWORD   i;
	...
	if(i < cTypes) 
		...
		sprintf(lpqt->sz,TEXT("select * from %s where %s not like 'a'"), lpqt->szTableName,
			lpqt->buf, lpqt->buf);
		rc = SQLExecDirect(hstmt, lpqt->sz, SQL_NTS);
		RETCHECK(SQL_SUCCESS, rc,szSQLEXECDIRECT);
		...
		for(i = 0;; i++) 
			...
			rc = SQLGetData(hstmt, 1, SQL_C_CHAR, lpqt->sz, sizeof(lpqt->sz),
				&cbValue);
			if(rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
				RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA);
			}
...
void   TestOJCap(QTSTRUCT  *lpqt)
{
	...
	if(RETCHECK(SQL_SUCCESS, rc,szSQLGETINFO))
	{
		...
		if(udw)
		{
			sprintf(lpqt->buf,TEXT("Undefined flags return from SQLGetInfo(...SQL_OJ_CAPABILITIES...) = %lX"), udw);
...
void   TestExtendedFetch(QTSTRUCT  *lpqt, FIELDINFO  *rgFields, UWORD cTypes)
{
	RETCODE rc=SQL_SUCCESS;
	WORD            fLevel2 = FALSE;
	DWORD           dwLen=0;
	...
	if (!(rgbValue = AllocateMemory(MAX_STRING_SIZE*sizeof(CHAR)))){
		DISPLAYERROR(szSQLEXTENDEDFETCH,TEXT("Insufficient Memory Available"));
	...
	sprintf(szQuery,TEXT("select %s from %s"),lpqt->szColNames, lpqt->szTableName);
	rc = SQLExecDirect(hstmt, szQuery, SQL_NTS);
	if((SQL_SUCCESS!=rc) && (SQL_SUCCESS_WITH_INFO!=rc)){
		RETCHECK(SQL_SUCCESS_WITH_INFO, rc, szSQLEXECDIRECT);
		goto abortxfetch;
	...
	for (row=0; row<uwRowSetSize; row++)
	{
		...
		for (iTableCol=0; iTableCol<cTypes; iTableCol++)
		{
			...
			iResultSetCol++;
			rc = SQLGetData(hstmt, iResultSetCol, SQL_C_CHAR, rgbValue, MAX_STRING_SIZE*sizeof(CHAR),
				&cbValue);
			if(rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
				RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA);
			...
			{
				sprintf(lpqt->buf, TEXT("Data Check Failed, Row %d, Table Col %d, DataType %d (%s)"),
				...
				sprintf(lpqt->buf, TEXT("Returned Data: (%s)"), rgbValue);
...
void   TestQuickRebind(QTSTRUCT * lpqt, FIELDINFO  *rgFields, UWORD cTypes,
									  DSTRUCT  *lpd)
{
	RETCODE rc;
	...
	/* Allocate an area for test data */
	if (!(rgbValue = AllocateMemory(MAX_STRING_SIZE * sizeof(SQLCHAR)))){
		DISPLAYERROR(szSQLEXTENDEDFETCH,TEXT("Insufficient Memory Available"));
		goto abortQuikRebind;
	}
	...
	for (iRowset=0; iRowset<=1; iRowset++)
	{
		...
		for (row=0; row<(uwRowsetSize*2)  ;row++,iRowsetRow++)
		{
			...
			for(iTableCol=0; iTableCol<cTypes; iTableCol++)
			{
				...
				iResultSetCol++;
				rc = SQLGetData(hstmt, iResultSetCol, SQL_C_CHAR, rgbValue, MAX_STRING_SIZE * sizeof(SQLCHAR),
					&cbValue);
				if(rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
					RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA);
...
void   TestSQLForeignKeys(QTSTRUCT  *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
...
void   TestSQLBrowseConnect(lpSERVERINFO lpSI)
{
	RETCODE rc=SQL_SUCCESS;
...
void   TestSQLMoreResults(QTSTRUCT  *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
...
void   TestSQLNativeSQL(QTSTRUCT  *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
	CHAR           szQuery[XLARGEBUFF];
	
	sprintf(szQuery,TEXT("select * from %s were 0 > 1"),lpqt->szTableName);
...
void   TestSQLDescribeParam(QTSTRUCT  *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
...
void   TestSQLNumParams(QTSTRUCT  *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
...
void   TestSQLParamOptions(QTSTRUCT *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
...
void   TestSQLPrimaryKeys(QTSTRUCT *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
...
void   TestSQLProcedureColumns(QTSTRUCT  *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
...
void   TestSQLColumnPrivileges(QTSTRUCT  *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
...
VOID  TestConnectAttr(HDBC hdbc,SDWORD fAttribute, UDWORD rgbValue,LPTSTR lpAttr,BOOL fReadOnlyAttr)
{
	RETCODE rc=SQL_SUCCESS;
	CHAR           szBuff[LARGEBUFF]=TEXT("");
	UDWORD  pvParam=0;
	...
	if(!FindError(SQL_HANDLE_DBC,szHYC00))
	{
		...
		if(!FindError(SQL_HANDLE_DBC,szHYC00))
		{
			...
			{
				sprintf(szBuff,TEXT("%s returned incorrect value"),lpAttr);
...
SWORD CreateDescRecord(SQLHDESC hdesc, SQLHSTMT hstmt, UWORD uwDescType)
{
	...
	switch(uwDescType) {
		case SQL_ATTR_APP_ROW_DESC:
			...
			rc=SQLBindCol(hstmt, 1, SQL_C_CHAR, rgBuf, sizeof(rgBuf), NULL);
			RETCHECK(SQL_SUCCESS, rc,szSQLBINDCOL);
			if (SQL_SUCCEEDED(rc))
				return 1;       
			break;
	...
	if (SQL_NULL_HDESC == hdesc2) {
		...
		if (rgDescInfo[uwDescField].fSettable & DescTypes[uwDescIndex].uwTypeMask) {
			...
			if (!rgDescInfo[uwDescField].size)
			{
				...
				cbValue2        = (_tcslen(pNewvalue)+1) * sizeof(CHAR); 
			}
		...
		if (!hdesc2 || (rgDescInfo[uwDescField].fSettable & DESC_ARD))
		{
				...
				SQL_DESC_DATA_PTR != rgDescInfo[uwDescField].uwDescField)
			{
				if (memcmp(DescBuf, DescBuf2, cbValue2)) {
					...
					sprintf(buf,TEXT("Compare error in %s field %s"),
...
void CheckDescriptor(SQLHDESC hdesc, SQLINTEGER swUpdateMode, SQLHSTMT hstmt, UWORD uwDescIndex)
{
	...
	if (SQL_SUCCESS == rc) {
			...
			for (iDescField=iStartField; iDescField < sizeof(rgDescInfo)/sizeof(rgDescInfo[0]); iDescField++) {
				...
				if (DESC_UPDATE_MODE_READ == swUpdateMode)
				{
					...
					if (RC_SUCCESSFUL(rc))
					{
						...
						if (!rgDescInfo[iDescField].size)
							cbValue2 = _tcslen((LPTSTR)DescBuf) * sizeof(SQLCHAR);  // For string fields

						// Make sure cbValue and cbValue2 agree
						if (cbValue != cbValue2)
						{
							sprintf(buf,TEXT("Compare error in %s field %s, expected")\
				...
				else
				{
					...
					rc = SQLSetDescField(hdesc, swDescRec, rgDescInfo[iDescField].uwDescField,
						...
						_tcslen((LPTSTR)rgDescInfo[iDescField].NewValue)*sizeof(CHAR));
		...
		sprintf(buf,TEXT("\t\tUnable to read record count from descriptor header."));
		ERRSHANDLE(SQL_HANDLE_DESC, hdesc, SQL_SUCCESS, rc, buf);
	}
}
...
void CompareDescriptor(SQLHDESC hdesc1, SQLHDESC hdesc2, UWORD uwDescIndex, CHAR * pszAPI)
{
	...
	// (hdesc2 is allowed to be NULL)
	if (SQL_NULL_HDESC == hdesc1) {
		sprintf(buf,TEXT("\t\thCompareDescriptor: hdesc1 handle was null!"));
		DISPLAYERROR(szSQLGETDESCFIELD, buf);
		return;
	}
	   ...
	   SQLPOINTER pData, SQLINTEGER *pcbValue, SQLINTEGER *pIndicator, SQLUSMALLINT iDescType)
{
	...
	{
		sprintf(buf,TEXT("%s: Unable to write record %d in %s"),
	...
	{
		// Read it again
		rc = SQLGetDescRec(hdesc, swDescRec, szName, sizeof(szName)/sizeof(CHAR),
			&cbName, &fType2, &fSubType2, &Length, &Precision, &Scale, &Nullable);

		sprintf(buf,TEXT("%s: Unable to read record %d in %s"),
...
void   TestGetDescField(QTSTRUCT * lpqt)
{
	...
	else
	{
		...
		sprintf(szQuery,szSELECTSTAR,lpqt->szTableName);
		rc = SQLExecDirect(hstmt, szQuery, SQL_NTS);
		RETCHECK(SQL_SUCCESS, rc,szSQLEXECDIRECT);
		...
		for (iDescType=0; iDescType < sizeof(DescTypes)/sizeof(DescTypes[0]); iDescType++)
		{
			...
			if (SQL_SUCCEEDED(rc)) 
			{
				...
				{
					sprintf(buf,TEXT("Expected record count %d, found %d"),
			...
			{
				// Unable to read record count from descriptor header, can't proceed
				sprintf(buf,TEXT("Unable to read record count from descriptor header for descriptor type %d"),
...
void   TestSetDescField(QTSTRUCT * lpqt)
{
	SQLRETURN rc=SQL_SUCCESS;
	SQLUSMALLINT iDescType;
...
void   TestGetDescRec(QTSTRUCT * lpqt)
{
	SQLRETURN rc=SQL_SUCCESS;
	SQLUSMALLINT iDescType;
	...
	else
	{
		...
		for (iDescType=0; iDescType < sizeof(DescTypes)/sizeof(DescTypes[0]); iDescType++)
		{
			...
			if (SQL_SUCCESS == rc) 
			{
				...
				for (swDescRec=0; swDescRec <= swDescRecCount+1; swDescRec++)
				{
						...
						if (swDescRec <= swDescRecCount)
						{
							...
							{
								sprintf(buf,TEXT("%s: Able to read record %d from %s"),
									szSQLGETDESCREC, swDescRec, DescTypes[iDescType].szDescName);
								ERRSHANDLE(SQL_HANDLE_DESC, hdesc1, SQL_ERROR, rc, buf);
							}
							else
							{
								sprintf(buf,TEXT("%s: Unable to read record %d from %s"),
									szSQLGETDESCREC, swDescRec, DescTypes[iDescType].szDescName);
								ERRSHANDLE(SQL_HANDLE_DESC, hdesc1, SQL_SUCCESS, rc, buf);
							}
						}
						...
						{
							sprintf(buf,TEXT("%s: Did not receive SQL_NO_DATA reading record %d from %s"),
								szSQLGETDESCREC, swDescRec, DescTypes[iDescType].szDescName);
			...
			{
				// Unable to read record count from descriptor header, can't proceed
				sprintf(buf,TEXT("\t\tUnable to read record count from descriptor header for %s"),
					DescTypes[iDescType].szDescName);
				ERRSHANDLE(SQL_HANDLE_DESC, hdesc1, SQL_SUCCESS, rc, buf);
			}
		}
...
void   TestSetDescRec(QTSTRUCT * lpqt)
{
	SQLRETURN rc=SQL_SUCCESS;
...
void   TestCopyDesc(QTSTRUCT * lpqt)
{
	SQLRETURN rc=SQL_SUCCESS;
	SQLUSMALLINT iDescType;
	SQLSMALLINT swExpRecCount;
...
//-----------------------------------------------------------------------
void   TestDescDefaults(QTSTRUCT * lpqt)
{
	SQLRETURN rc=SQL_SUCCESS;
	SQLINTEGER cbValue;
			...
			for (iDescField=iStartField; iDescField < sizeof(rgDescInfo)/sizeof(rgDescInfo[0]); iDescField++) {
				...
				if (rgDescInfo[iDescField].fDefault & DescTypes[iDescType].uwTypeMask)
				{
					...
					if (RC_SUCCESSFUL(rc))
					{
						...
						{
							// Compare error, print message
							sprintf(buf,TEXT("Error in %s field %s default value"),
...
//-----------------------------------------------------------------------
void   TestUseDesc(QTSTRUCT * lpqt, FIELDINFO  *rgFields)
{
	LPTSTR   pch=NULL;
	SDWORD crow=0;
	CHAR   szQuery[XLARGEBUFF]=TEXT("");
	...
	SWORD   ccol;
	CHAR * rgbMemBuf=NULL, * rgbValue=NULL;
	...
	if (Supported(SQL_API_SQLSETDESCFIELD))
	{
		...
		rgbMemBuf=AllocateMemory(ccol*MAX_STRING_SIZE*sizeof(CHAR));
...
//                              rc=SQLBindParameter(hstmt, paramno, SQL_PARAM_INPUT, SQL_C_CHAR, 
		...
		{
			// Bind to next column
			rc = DescBindCol(hstmt, icol, SQL_C_CHAR, rgbValue, MAX_STRING_SIZE*sizeof(CHAR), pcbValue);
//                      rc = SQLBindCol(hstmt, icol, SQL_C_CHAR, rgbValue, MAX_STRING_SIZE*sizeof(CHAR), pcbValue);
...
void   TestEnvAttr()
{
	...
	{
		// Default value wrong
		sprintf(buf, TEXT("Value was not %d!"), DEFAULT_ODBC_VERSION);
...
//-----------------------------------------------------------------------
SWORD   ExecEndTran(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,UWORD cTypes,
...
//-----------------------------------------------------------------------
void   TestEndTran(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,UWORD cTypes,
										DSTRUCT  *lpd)
{
	SWORD                           i;
	RETCODE                         rc;
...
//-----------------------------------------------------------------------
void   TestBindParam(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,UWORD cTypes,
										DSTRUCT  *lpd)
{
	RETCODE                         rc;
	...
	else
	{
		...
		rc = SQLBindParam(hstmt,1,SQL_C_CHAR, rgFields[0].wSQLType, rgFields[0].precision,
...
void  TestFetchScroll(QTSTRUCT *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
	CHAR           szSave[MAX_QUERY_SIZE];
	DWORD           dwLen=0;
	SDWORD  cbValue=0;
	...
	else
	{
		...
		rc = SQLBindCol(hstmt, 1, SQL_C_CHAR, lpqt->sz, sizeof(lpqt->sz),
			&cbValue);
		RETCHECK(SQL_SUCCESS, rc, szSQLBINDCOL);
...
void   qtDisplayError(LPTSTR szFunction, LPTSTR buf, LPTSTR szFile, int iLine)
{
	...
	sprintf(szTmp,TEXT("\t\t\t%s: %d"), szFile, iLine);
	szWrite(szTmp, TRUE);
...
LPTSTR   qtMakeData(int row, int col,FIELDINFO  *rgField, LPTSTR buf)
{
	int i;
	CHAR szColNum[10];
	CHAR tmpbuf[256];
	...
	switch(rgField->wSQLType) {
		...
		case SQL_VARBINARY:
		case SQL_LONGVARBINARY:
			if(rgField->precision < (4*sizeof(CHAR)) )
				sprintf(buf,TEXT("%d"), 1);
			else
				sprintf(buf,TEXT("%d%d"), row, row);
			break;
		case SQL_DECIMAL:
		case SQL_NUMERIC:
		case SQL_REAL:
		...
		case SQL_TIME:
		case SQL_TYPE_TIME:
			sprintf(buf,TEXT("{t '01:%02d:%02d'}"), row % 60, row % 60);
			break;
		case SQL_DATE:
		case SQL_TYPE_DATE:
			sprintf(buf,TEXT("{d '1994-%02d-%02d'}"), (row % 12) + 1, (row % 30) + 1);
			break;
		case SQL_TIMESTAMP:
		case SQL_TYPE_TIMESTAMP:
			// precision <= 16 means the timestamp column doesn't have enough precision to hold seconds
			if ( rgField->precision > 16 )
				sprintf(buf,TEXT("{ts '1994-%02d-%02d 01:%02d:%02d'}"), (row % 12) + 1, (row % 30) + 1, row % 60, row % 60);
			else
				sprintf(buf,TEXT("{ts '1994-%02d-%02d 01:%02d:%02d'}"), (row % 12) + 1, (row % 30) + 1, row % 60, 0);
			break;
		...
		case SQL_GUID:
			...
			sprintf(buf, TEXT("%.8s-%.4s-%.4s-%.4s-%.12s"), tmpbuf, tmpbuf, tmpbuf, tmpbuf, tmpbuf);
			break;
...
VOID  TestNumResCols(SDWORD cColsSelected,QTSTRUCT *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
	SWORD           wNum;
...
VOID  TestCursorName(QTSTRUCT *lpqt)
{
	RETCODE rc=SQL_SUCCESS;
	CHAR           szCursorName[MEDBUFF];
	SWORD           cb=0;
	...
	rc = SQLGetCursorName(hstmt, szCursorName, sizeof(szCursorName)/sizeof(CHAR), &cb);
...
MY_EXPORT BOOL AutoTestName(LPTSTR szName, UINT  * cbTestCases)
{
	static CHAR szTestName[] = TEXT("Another Quick Test");
...
//-----------------------------------------------------------------------
MY_EXPORT BOOL AutoTestDesc(UWORD iTest, LPTSTR szName, LPTSTR szDesc)
{
...
MY_EXPORT void AutoTestFunc(lpSERVERINFO pTestSource)
{
