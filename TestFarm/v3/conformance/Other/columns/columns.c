...
int main(int argc, argv_t argv)
{
  ...
  STRCPY(stmt, "DROP TABLE ESC_APED");
  SPRINTF(buffer, "%s"CRLF, stmt);
  PRINTF(buffer);
  retcode = SQLExecDirect (hstmt, stmt, SQL_NTS);
  STRCPY(stmt, "create table ESC_APED ( ESC_APED char (10))");
  SPRINTF(buffer, "%s"CRLF, stmt);
  PRINTF(buffer);
  retcode = SQLExecDirect (hstmt, stmt, SQL_NTS);
  ...
  retcode = SQLColumns (hstmt, NULL, (SWORD) 0, ODBCUSER, (SWORD) STRLEN(ODBCUSER), "ESC\\_APED", SQL_NTS, "%", SQL_NTS);
  ...
  MakeLongName(ODBCUSER, szLongTableName, cbTableNameMax);
  ...
  retcode = SQLColumns (hstmt, NULL, (SWORD) 0, ODBCUSER, (SWORD) STRLEN(ODBCUSER), szLongTableName, SQL_NTS, szLongTableName, SQL_NTS);
  ...
  retcode = SQLColumns (hstmt, NULL, (SWORD)0, ODBCUSER, (SWORD)STRLEN(ODBCUSER), "\\_UNKNOWN", SQL_NTS, "%", SQL_NTS);
  ...
  retcode = SQLFreeStmt(hstmt, SQL_DROP);
  SPRINTF(buffer, " SQLFreeStmt (drop) retcode: %d"CRLF, retcode);
  PRINTF(buffer);
  ERROR_CHECK(hstmt);
