/*!**********************************************************************
  ...
  description:  Retrieve bookmarks that are bound with SQLBindCol.
                Use SQL_INTEGER bookmarks (the old fashioned 2.5 way).
    ...
    for (i=0; i < (sizeof(TestValues) / sizeof( TestTableStruct)); i++ ) {
        ...
        default: {
            ...
            if (retcode != SQL_NO_DATA_FOUND) {
                SPRINTF( textBuffer,
                         "%d \t %s \t %d \t \n",
                         bookmarkValue, charValue, intValue );
                PRINTF( textBuffer );
                  ...
                  SWORD   optimizeCursor )
{
    ...
    retcode = SQLBindCol( *phstmt, 0,
                          SQL_C_BOOKMARK, &bookmarkValue,
                          sizeof(bookmarkValue), &bookmarkIndi );
    SPRINTF( textBuffer,
