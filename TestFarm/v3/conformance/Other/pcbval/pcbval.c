/*!**********************************************************************
  ...
  description:  SQLBindCol, SQLFetch, when size of return buffer is 0
...
UCHAR szDefFLOAT[] = "5.5555"; SFLOAT sDefFLOAT = (SFLOAT)SQL_C_FLOAT; SQLLEN cbDefFLOAT = 0;
...
VOID rgbValueShow(SWORD fCType, PTR rgbValue, SQLLEN *pcbValue)
{
   ...
   switch (fCType) {
	 ...
	 len = (pcbValue == NULL) ? STRLEN(rgbValue) : *pcbValue;
         if (len < 0)
	    SPRINTF(localbuf, "SQL_C_CHAR : SQL_NO_TOTAL");
	 else if (len < BUFSIZE)
	 ...
	 UDWORD bufsize;
	 UCHAR hchar[5];
	 UCHAR FAR *p, *q = (char*)&di;     /* for little/big endian test */
	 p = (UCHAR FAR *)rgbValue;
	 bufsize = BUFSIZE / 3;
	 STRCPY(localbuf, "SQL_C_BINARY = ");
	 len = (pcbValue == NULL) ? STRLEN(rgbValue) : *pcbValue;
         if (len < 0)
	    SPRINTF(localbuf, "SQL_C_BINARY : SQL_NO_TOTAL");
	 else
	 if (len < bufsize) {
           if (len == sizeof(double)  &&  *q == 0) {  /* endian? */
             q = p;
             p += sizeof(double) - 1;
             di = -1;
           }
	    for ( i=1; i <= len; i++, p+=di) {
	       SPRINTF(hchar, "%3x", *p);
