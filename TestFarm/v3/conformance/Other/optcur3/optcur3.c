 ...
 * wird. Es wird das fetchen mit groﬂer keyset-size getestet.
 * insbesondere keyset-size um die 250 bzw. Vielfaches von 250
...
TestTableStruct TestValues[] = {
      { SQL_SELECT_NEW, 249, "", SQL_CURSOR_KEYSET_DRIVEN,
        SQL_CONCUR_READ_ONLY },
      { SQL_FETCH_NEXT, 0, "", 0, 0 },
      { SQL_FETCH_NEXT, 0, "", 0, 0 },
      { SQL_FETCH_RELATIVE, -10, "", 0, 0 },
      { SQL_FETCH_RELATIVE, +240, "", 0, 0 },
      { SQL_FETCH_NEXT, 0, "", 0, 0 },
      { SQL_SELECT_NEW, 250, "", SQL_CURSOR_KEYSET_DRIVEN,
        SQL_CONCUR_READ_ONLY },
      { SQL_FETCH_NEXT, 0, "", 0, 0 },
      { SQL_FETCH_NEXT, 0, "", 0, 0 },
      { SQL_FETCH_RELATIVE, -10, "", 0, 0 },
      { SQL_FETCH_RELATIVE, +240, "", 0, 0 },
      { SQL_FETCH_NEXT, 0, "", 0, 0 },
      { SQL_SELECT_NEW, 251, "", SQL_CURSOR_KEYSET_DRIVEN,
        SQL_CONCUR_READ_ONLY },
      { SQL_FETCH_NEXT, 0, "", 0, 0 },
      { SQL_FETCH_NEXT, 0, "", 0, 0 },
      { SQL_FETCH_RELATIVE, -10, "", 0, 0 },
      ...
      { SQL_FETCH_NEXT, 0, "", 0, 0 }
      /*
      { SQL_SELECT_NEW, 7, "", SQL_CURSOR_KEYSET_DRIVEN, SQL_SCCO_LOCK },
      { SQL_FETCH_NEXT, 0, "", 0, 0 },
      { SQL_FETCH_ABSOLUTE, 247, "", 0, 0 },
      { SQL_FETCH_NEXT, 0, "", 0, 0 },
      { SQL_FETCH_RELATIVE, -10, "", 0, 0 },
      { SQL_FETCH_ABSOLUTE, 748, "", 0, 0 },
      { SQL_FETCH_NEXT, 0, "", 0, 0 }
      */
};
...
int main(int argc, argv_t argv)
{
  ...
  for (p=TestValues; NumTestValues > 0 ; p++, NumTestValues--) {
	...
	default:{
	   ...
	   SPRINTF(buffer, " SQLExtendedFetch '%s' (%d) retcode: '%s'"CRLF, cnvFetchType(p->fFetchType), (int) p->irow, cnvret(retcode));
	   PRINTF(buffer);
	   ERROR_CHECK(hstmt);
	   PrintArrSet(rowset);
	   break;
...
int DoSelect(int rowset, DWORD fSqlCursorType, DWORD fSqlConcur)
{
  ...
  PRINTF(buffer);
  ERROR_CHECK(hstmt);
  STRCPY(stmt, "select * from optcur3");
  SPRINTF(buffer, "%s"CRLF, stmt);
  PRINTF(buffer);
  retcode = SQLExecDirect (hstmt, stmt, STRLEN(stmt));
