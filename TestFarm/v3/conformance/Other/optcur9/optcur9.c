 ...
 * wird.
  description: Feth hinter die Ergebnismenge und dann fetch relattive -14
               mit SQL_ATTR_OPTIMIZE_CURSOR. Beim Massenfetch werden
               nur 10 Zeilen geliefert, es wird also beim fetch relative
               -14 ein fetch Kommando zum Kern geschickt.
               PTS 1102809
...
{ SQL_FETCH_ABSOLUTE, -2, "", 0, 0 },
{ SQL_FETCH_NEXT, 0, "", 0, 0 },
{ SQL_FETCH_NEXT, 0, "", 0, 0 },
{ SQL_FETCH_RELATIVE, -14, "", 0, 0 }
};
...
int main(int argc, argv_t argv)
{
  ...
  STRCPY(stmt, "create table "TABLE"(i integer, c varchar(3000))");
  SPRINTF(buffer, "%s"CRLF, stmt);
  PRINTF(buffer);
  retcode = SQLExecDirect (hstmt, stmt, STRLEN(stmt));
...
int DoSelect(int rowset, DWORD fSqlCursorType, DWORD fSqlConcur)
{
  ...
  STRCPY(stmt, "select * from optcur9");
  SPRINTF(buffer, "%s"CRLF, stmt);
  PRINTF(buffer);
  retcode = SQLExecDirect (hstmt, stmt, STRLEN(stmt));
