...
int main(int argc, argv_t argv)
{
  ...
  retcode = SQLConnect(hdbc, DBNODE, (SWORD) SQL_NTS,
     USER1, (SWORD) SQL_NTS, PASSW1, (SWORD) SQL_NTS );
  SPRINTF(buffer, " 1. SQLConnect retcode: %d"CRLF, retcode);
  PRINTF(buffer);
  ERROR_CHECK(henv, hdbc, SQL_NULL_HSTMT);

  retcode = SQLConnect(hdbc2, DBNODE, (SWORD) SQL_NTS,
     USER2, (SWORD) SQL_NTS, PASSW2, (SWORD) SQL_NTS );
  SPRINTF(buffer, " 2. SQLConnect retcode: %d"CRLF, retcode);
  PRINTF(buffer);
  ERROR_CHECK(henv, hdbc2, SQL_NULL_HSTMT);

  retcode = SQLAllocStmt (hdbc, phstmt);
  retcode = SQLExecDirect (hstmt, "create table transact1 (id int)", SQL_NTS);
  SPRINTF(buffer, " 1. SQLExecDirect retcode: %d"CRLF, retcode);
  PRINTF(buffer);
  retcode = SQLExecDirect (hstmt, "select * from transact1", SQL_NTS);
  SPRINTF(buffer, " 1. SQLExecDirect retcode: %d"CRLF, retcode);
  PRINTF(buffer);
  ...
  retcode = SQLExecDirect (hstmt, "create table transact2 (name char(10))", SQL_NTS);
  SPRINTF(buffer, " 2. SQLExecDirect retcode: %d"CRLF, retcode);
  PRINTF(buffer);
  retcode = SQLExecDirect (hstmt, "select * from transact2", SQL_NTS);
  SPRINTF(buffer, " 2. SQLExecDirect retcode: %d"CRLF, retcode);
  PRINTF(buffer);
  ...
  retcode = SQLTransact(henv, SQL_NULL_HDBC, SQL_ROLLBACK);
  SPRINTF(buffer, " global SQLTransact retcode: %d"CRLF, retcode);
  PRINTF(buffer);
  ERROR_CHECK(henv, hdbc, SQL_NULL_HSTMT);

  retcode = SQLTransact(SQL_NULL_HENV, hdbc2, SQL_COMMIT);
  SPRINTF(buffer, " SQLTransact (hdbc2) retcode: %d"CRLF, retcode);
  PRINTF(buffer);
  ERROR_CHECK(henv, hdbc, SQL_NULL_HSTMT);

  retcode = SQLTransact(SQL_NULL_HENV, hdbc, SQL_COMMIT);
  SPRINTF(buffer, " SQLTransact (hdbc) retcode: %d"CRLF, retcode);
  PRINTF(buffer);
  ERROR_CHECK(henv, hdbc, SQL_NULL_HSTMT);
