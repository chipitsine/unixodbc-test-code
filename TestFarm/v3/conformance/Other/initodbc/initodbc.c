...
RETCODE command (SQLHDBC hdbc, UCHAR *stmt, SWORD cmd_mode, SDWORD stmt_len);
...
int main(int argc, argv_t argv)
{
    ...
    STRCPY(stmt, "CREATE USER ");
    STRCAT(stmt, ODBCUSER);
    STRCAT(stmt, " password ");
    STRCAT(stmt, ODBCPASSW);
    ...
    STRCPY(stmt, "create table odbcdriverver (version char (100))");
    retcode = command( hdbc, stmt, CMD_NO_RESULT, SQL_NTS);

    /*
    STRCPY(stmt, "insert into odbcdriverver values('");
    STRCAT(stmt, driverver);
    STRCAT(stmt, "')");
    retcode = command( hdbc, stmt, CMD_NO_RESULT, SQL_NTS);
    */

    retcode = command( hdbc, "create table atmp (f1 fixed(5), c1 char (10))", CMD_NO_RESULT, SQL_NTS);
    ERROR_CHECK(hstmt);

    retcode = command( hdbc, "insert atmp values (1, 'ABCD')", CMD_NO_RESULT, SQL_NTS);
    ERROR_CHECK(hstmt);

    retcode = SQLGetInfo(hdbc, SQL_MAX_USER_NAME_LEN, &cbUserNameMax, sizeof(cbUserNameMax), NULL);


    STRCPY(stmt, "CREATE USER \"");
    MakeLongName(ODBCUSER, stmt+STRLEN(stmt), cbUserNameMax);
    STRCAT(stmt, "\" password \"");
    MakeLongName(ODBCPASSW, stmt+STRLEN(stmt), cbUserNameMax);
    STRCAT(stmt, "\" resource not exclusive");
