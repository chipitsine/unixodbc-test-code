...
#define ERROR_CHECK(henvx, hdbcx, hstmtx) if (retcode != SQL_SUCCESS) {\
			   ...
			   PRINTF(buffer);\
		  SPRINTF(buffer, " sql_code: %ld '%s'"CRLF, esq_sql_code, error_txt);\
		  PRINTF(buffer);\
                  switch (retcode) {\
		  case(100) :\
			  break;\
...
int main(int argc, argv_t argv)
{
  ...
  for ( i=0; i < MAX_CONNECTIONS; i++) {
     ...
     SPRINTF(buffer, " %d. SQLAllocConnect retcode: %d"CRLF, i, retcode);
     PRINTF(buffer);
     ERROR_CHECK(henv, hdbc, SQL_NULL_HSTMT);
  }

  for ( i=0; i < MAX_CONNECTIONS/2; i++) {
  retcode = SQLConnect(hdbc[i],
		       DBNODE,
		       (SWORD) SQL_NTS,
		       USER1,
		       (SWORD) SQL_NTS,
