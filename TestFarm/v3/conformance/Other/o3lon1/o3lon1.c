/*!**********************************************************************
  ...
  description: Read long data with SQLGetData. PTS 1102510
               - two long columns
               - first read read only the first 1024 bytes of both longs
               - fetch again
               - then read the rest of both longs
...
int main(int argc, argv_t argv) {
    ...
    retcode = CreateTable( hdbc, PROGNAME,
                           "( a int, b long ascii, c long byte )" );
    if (retcode != SQL_SUCCESS) {
        goto exit;
    }; /* if */
...
RETCODE ShowTable( HDBC hdbc )
{
    ...
    retcode = SQLBindCol( hstmt, 1,
                          SQL_C_SLONG, &intValue, 0, &intIndi );
    SPRINTF( textBuffer,
    ...
    retcode = SQLGetData( hstmt, 2, SQL_C_CHAR, charValue, 1024, &charIndi );
    SPRINTF( textBuffer,
    ...
    retcode = SQLGetData( hstmt, 3, SQL_C_BINARY, binValue, 1024, &binIndi );
    SPRINTF( textBuffer,
             "ShowTable\t -- SQLGetData \t\t\t retcode: %s"CRLF,
             cnvret( retcode ));
    PRINTF( textBuffer );
    ...
    retcode = SQLGetData( hstmt, 2, SQL_C_CHAR, charValue, CHAR_LEN,&charIndi );
    SPRINTF( textBuffer,
             "ShowTable\t -- SQLGetData \t\t\t retcode: %s"CRLF,
             cnvret( retcode ));
    PRINTF( textBuffer );
    ...
    retcode = SQLGetData( hstmt, 3, SQL_C_BINARY,binValue, CHAR_LEN, &binIndi );
    SPRINTF( textBuffer,
             "ShowTable\t -- SQLGetData \t\t\t retcode: %s"CRLF,
             cnvret( retcode ));
    PRINTF( textBuffer );
