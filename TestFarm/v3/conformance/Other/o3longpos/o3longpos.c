...
int main(int argc, argv_t argv) {
    ...
    retcode = InsertLong( hstmt, 10000 );
    if (retcode != SQL_SUCCESS) {
        goto exit;
    }; /* if */
    retcode = InsertLong( hstmt, 32666 );
    if (retcode != SQL_SUCCESS) {
        goto exit;
    }; /* if */

    retcode = InsertLong( hstmt, 33000 );
    if (retcode != SQL_SUCCESS) {
        goto exit;
    }; /* if */

    retcode = InsertLong( hstmt, 68000 );
    if (retcode != SQL_SUCCESS) {
        goto exit;
    }; /* if */
    ...
    retcode = ShowTable( hstmt, 0 );
    if (retcode != SQL_SUCCESS) {
        goto exit;
    }; /* if */

    retcode = ShowTable( hstmt, 100 );
    if (retcode != SQL_SUCCESS) {
        goto exit;
    }; /* if */

    retcode = ShowTable( hstmt, 33000 );
    if (retcode != SQL_SUCCESS) {
        goto exit;
    }; /* if */

    retcode = ShowTable( hstmt, 34000 );
    if (retcode != SQL_SUCCESS) {
        goto exit;
    }; /* if */

    retcode = ShowTable( hstmt, 65000 );
    if (retcode != SQL_SUCCESS) {
        goto exit;
    }; /* if */

    retcode = ShowTable( hstmt, 90000 );
    if (retcode != SQL_SUCCESS) {
        goto exit;
    }; /* if */
...
RETCODE ShowTable( HSTMT hstmt, UDWORD longPos )
{
    ...
    do {
        ...
        if (retcode == SQL_SUCCESS || retcode == SQL_SUCCESS_WITH_INFO) {
            ...
            retcode = SQLGetData( hstmt, 1, SQL_C_BINARY, binValue, CHAR_LEN,
                                  &binIndi );
            SPRINTF( textBuffer,
