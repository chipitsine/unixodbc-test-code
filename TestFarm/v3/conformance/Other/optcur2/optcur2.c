 ...
 * wird. Es wird das relative fetchen vor bzw. hinter die Ergebnismenge
 * getestet.
...
{ SQL_SELECT_NEW, 3, "", SQL_CURSOR_KEYSET_DRIVEN, SQL_CONCUR_READ_ONLY },
...
int DoSelect(int rowset, DWORD fSqlCursorType, DWORD fSqlConcur)
{
  ...
  STRCPY(stmt, "select * from optcur2");
  SPRINTF(buffer, "%s"CRLF, stmt);
  PRINTF(buffer);
  retcode = SQLExecDirect (hstmt, stmt, STRLEN(stmt));
