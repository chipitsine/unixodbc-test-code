/*!**********************************************************************
  ...
  description: testing SQLFetchScroll
...
{ SQL_FETCH_LAST, 0, "" },
{ SQL_FETCH_RELATIVE, -1, "" },
{ SQL_FETCH_RELATIVE, 3, "" }
*/
};

RETCODE InsertValues( HSTMT hstmt );

RETCODE TestFetchScroll( HSTMT *phstmt );
                  ...
                  DWORD   fSqlConcur )
{
    ...
    SPRINTF( textBuffer, "DoSelectt -- SQLBindCol(1) \t\t retcode: %s"CRLF,
             cnvret( retcode ));
    PRINTF( textBuffer );
    ErrorCheck( SQL_HANDLE_STMT, *phstmt );
    if (retcode != SQL_SUCCESS) {
