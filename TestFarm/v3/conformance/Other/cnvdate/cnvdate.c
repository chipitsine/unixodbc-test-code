...
SWORD sSMALLINT;
SQLLEN cbDOUBLE, cbSMALLINT, cbCHAR, cbBINARY, cbLONGVARCHAR, cbDECIMAL;
...
UCHAR szDefSTAMP[] = "1994-10-10 15:20:23.000000";
...
static TestTypesStruct TestType[] = {
...
{SQL_DATE, "SQL_DATE", "create table "TABLE" (n integer, col date)", TestType},
{SQL_TIME, "SQL_TIME", "create table "TABLE" (n integer, col time)", TestType},
{SQL_TIMESTAMP, "SQL_TIMESTAMP", "create table "TABLE" (n integer, col timestamp)", TestType},
{0, NULL, NULL}
};
...
VOID rgbValueShow(SWORD fCType, PTR rgbValue, SQLLEN *pcbValue, SWORD fSqlTypeIn);
...
SQLLEN *pcbValueOut)
{
   ...
   PRINTF(buffer);
   rgbValueShow(fCTypeIn, rgbValueIn, pcbValueIn, fSqlTypeIn);
   ...
   if (retcode == SQL_SUCCESS) {
	 ...
	 PRINTF(buffer);
	 rgbValueShow(fCTypeOut, rgbValueOut, pcbValueOut, fSqlTypeIn);
      }
   }
   retcode = SQLFreeStmt(hstmt, SQL_CLOSE);
   ERROR_CHECK(hstmt);
   ...
   switch (fCType) {
	 ...
	 if (len < LOCALBUFSIZE)
	    ...
	    SPRINTF(localbuf, "SQL_C_CHAR = '%.*s"CRLF"........"CRLF"%.*s'",
	       LOCALBUFSIZE/3, rgbValue, LOCALBUFSIZE/3, ((char *)rgbValue+len-LOCALBUFSIZE/3));
	 break;
      }
      case (SQL_C_LONG): {
	 ...
	 if (fSqlTypeIn == SQL_TIME) {
	    SPRINTF(localbuf, "SQL_C_TIMESTAMP = (Current Date) GREPWEG(%d,%d,%d,%d,%d,%d,%d)", sTS->year, sTS->month, sTS->day,
	 ...
	 if (len < bufsize) {
	    for ( i=1; i <= len; i++,p++) {
	       SPRINTF(hchar, "%3x", (unsigned char)*p);
	       STRCAT(localbuf, hchar);
	    }
	 }
	 else {
	    for ( i=1; i <= bufsize/3; i++,p++) {
	       SPRINTF(hchar, "%3x", (unsigned char)*p);
	       STRCAT(localbuf, hchar);
	    }
	    STRCAT(localbuf, CRLF"........"CRLF);
	    for ( i=len-bufsize/3; i <= len; i++,p++) {
	       SPRINTF(hchar, "%3x", (unsigned char)*p);
	       STRCAT(localbuf, hchar);
	    }
	 }
